...
#include "Constant.h"
	...
	class BoundingBox {
	public:
		BoundingBox(const Vector3d &minPos = Vector3d(0, 0, 0), const Vector3d &maxPos = Vector3d(0, 0, 0))
			: m_min(minPos)
			, m_max(maxPos)
		...
		inline bool CheckIntersection(const Ray &ray, double &distance) const {
			Vector3d t_min(INF, INF, INF), t_max(-INF, -INF, -INF);
			...
			double ray_start_array[3] = { ray.pos.x(), ray.pos.y(), ray.pos.z() };
