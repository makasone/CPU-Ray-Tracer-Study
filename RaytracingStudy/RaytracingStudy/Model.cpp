...
using namespace std;
	...
	void Model::SetTransform(const Vector3d &pos, const Vector3d &scale, const MatrixXd& rot) {
		m_position = pos;
	...
	bool Model::ReadFromObj(const std::string &filename) {
		...
		materialNames[defaultMaterialName] = Material(Material::REFLECTION_TYPE_LAMBERT, Vector3d(0, 0, 0), Vector3d(0.99, 0.99, 0.99));
		m_meshes[materialNames[defaultMaterialName]] = PolygonList();

		string currentMaterialName = defaultMaterialName;
		string currentGroupName;

		vector<Vector3d> verticesInGroup;
		vector<Vector3d> normalsInGroup;
		vector<Vector3d> uvCoordinatesInGroup;
		...
		while (!ifs.eof()) {
			...
			if (line.find("#") == 0) {
				...
				// material name
				//static int index=0;
				//materialNames[line.substr(string("mtllib ").length())] = Material(Material::REFLECTION_TYPE_LAMBERT, Vector3d(0,0,0), Vector3d(0.999+index*0.001,0.99,0.99));
				//index++;
				...
				if (vertex.size() != 3) {
					...
					verticesInGroup.push_back(Vector3d(atof(vertex[0].c_str()), atof(vertex[1].c_str()), atof(vertex[2].c_str())));
				}
			}
				...
				if (vertex.size() != 2) {
					...
					uvCoordinatesInGroup.push_back(Vector3d(atof(vertex[0].c_str()), atof(vertex[1].c_str()), 0.0));
				}
			}
				...
				if (vertex.size() != 3) {
					...
					normalsInGroup.push_back(Vector3d(atof(vertex[0].c_str()), atof(vertex[1].c_str()), atof(vertex[2].c_str())));
				}
			}
	...
	bool Model::LoadMaterialFile(const std::string &filename, std::unordered_map<string, Material> &materials) {
		...
		while (!ifs.eof()) {
			...
			if (line.find("#") == 0) {
				...
				diffuse = Vector3d(atof(values[0].c_str()), atof(values[1].c_str()), atof(values[2].c_str()));
			}
			else if (line.find("Ks ") == 0) {
				// specular color
				vector<string> values(Utils::split(line.substr(string("Ks ").length()), ' '));
				assert(values.size() == 3);
				specular = Vector3d(atof(values[0].c_str()), atof(values[1].c_str()), atof(values[2].c_str()));
			}
	...
	std::vector<Model::PolygonPtr> Model::load4verticesFace(const vector<string> &face, const vector<Vector3d> &verticesInGroup,
		const vector<Vector3d> &normalsInGroup,
		const vector<Vector3d> &uvCoordinatesInGroup, const Material &mat) {
	...
	Model::PolygonPtr Model::Load3verticesFace(const vector<string> &face, const vector<Vector3d> &verticesInGroup,
		const vector<Vector3d> &normalsInGroup,
		const vector<Vector3d> &uvCoordinatesInGroup, const Material &mat) {
		Vector3d vec[3], normals[3];
		...
		if (averaged_normal.x() == 0 && averaged_normal.y() == 0 && averaged_normal.z() == 0) {
			...
			ret_p = (new Polygon(vec[0], vec[1], vec[2], Polygon::CalculateNormal(vec[0], vec[1], vec[2]), mat, Vector3d(0, 0, 0)));
		}
		else {
			averaged_normal /= 3.0;
			ret_p = (new Polygon(vec[0], vec[1], vec[2], averaged_normal*-1, mat, Vector3d(0, 0, 0)));
		}
